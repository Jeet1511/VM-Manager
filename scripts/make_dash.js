...
const newCard = `_renderVMCard(vm) {
...
code = code.replace(/_renderVMCard\(vm\) \{[\s\S]*?(?=  _openCloneModal)/, newCard + "\n\n");

// Replace overlay handlers
code = code.replace(/overlay\.onclick = close;/g, "overlay.onclick = close; const escHandler = (e) => { if (e.key === \\\"Escape\\") { close(); document.removeEventListener(\\\"keydown\\\", escHandler); } }; document.addEventListener(\\\"keydown\\\", escHandler);");
