...
contextBridge.exposeInMainWorld('vmInstaller', {
  ...
  deleteVM: (name, options) => ipcRenderer.invoke('vm:delete', name, options),
  getFileSize: (filePath) => ipcRenderer.invoke('util:fileSize', filePath),
  editVM: (name, settings) => ipcRenderer.invoke('vm:edit', name, settings),
  editVMXml: (name, settings) => ipcRenderer.invoke('vm:editXml', name, settings),
  getVMXmlSettings: (name) => ipcRenderer.invoke('vm:getXmlSettings', name),
